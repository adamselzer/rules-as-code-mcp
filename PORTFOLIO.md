...
logic as tools an AI agent can call. This is the newest of the four build
...
A hallucinated eligibility answer becomes a wrongful denial of food or medical
coverage. Agencies, auditors, and courts can review code and cited rules. They
cannot review a model's hidden reasoning. So the real question in the public sector
is where the model stops and the inspectable code starts. This project answers
...
3. **Reproducible determination ids, a hash of the inputs.**
...
4. **Scope resolved from the request context.** A model
...
5. **An honest stub for `lookup_policy`.** The policy lookup
